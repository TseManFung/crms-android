...
class rfidScanner {
    ...
    fun readTagWithFilter(readBank: Int): String {
        ...
        var TagData = this.scanner.readData(password,filterBank!!,filterPtr,filterCnt,filterData!!,readBank,readPtr,readCnt)
        if (TagData != null) {
            return TagData
        } else {
            TagData = this.scanner.readData(defaultPassword,filterBank!!,filterPtr,filterCnt,filterData!!,readBank,readPtr,readCnt)
            if (TagData != null) {
                return TagData
            } else {
                throw Exception("Failed to read tag from ${bankErrorMsg(readBank)}")
